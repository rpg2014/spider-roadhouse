...
export const EditEntry = (initalState: JournalEntryProps & editEntryProps) => {
  ...
  const { data, error, isPending, run, isFulfilled } = useFetch<IEditResponse>(
    SPIDERMAN_BASE_URL + JOURNAL + '/' + initalState.id,
    options
  );
  ...
  const getSubmitButton = () => {
    ...
    return (
      <button className="btn btn-dark btn-outline-success float-right small-font" onClick={handleSubmit}>
  ...
  return (
    <form className="form-group card text-white bg-dark mb-3 mx-3" onSubmit={(event) => console.log(event)}>
      <div className="card-header container-fluid text-left align-middle py-2 px-3">
        <div className="form-row">
          <div className="col-6 mx-auto">
            <input
              readOnly
              className="form-control-plaintext text-white bg-dark border-dark rounded"
              id="dateTime"
              value={dateTime.toTimeString()}
            />
          </div>

          <div className="col-6">
            <button
              className="btn btn-dark btn-outline-light small-font float-right mx-2 "
              onClick={initalState.toggleEntryMode}
            >
              Cancel
            </button>
            {getSubmitButton()}
      ...
      <div className="card-text text-left w-100 overflow-hidden height-300 p-2">
        <textarea
          className="form-control-sm w-100 h-100 bg-dark text-light overflow-hidden"
          id="text"
          rows={3}
          value={text}
