...
export const JournalList = () => {
  ...
  if (entriesState.isError && entriesState.errorData) {
    return <ErrorAlert errorDetail={entriesState.errorData} />;
  }
  ...
  if (entriesState.data && entriesState.data.length > 0) {
    ...
    return (
      <button className="btn  btn-outline-dark mx-auto py-3" onClick={toggleNew}>
  ...
  return (
    <div className="row center max_width mx-auto">
      {getSpinner()}
      <div className="container-fluid rounded">{list}</div>
    </div>
  );
};
