...
export class StartStopButton extends React.Component<StartStopButtonProps> {
  ...
  render() {
    if (this.props.serverActionStatus.isFetching) {
      return <div className="spinner-grow align-middle text-light"></div>;
    }

    if (
      (this.props.serverActionStatus.isError && this.props.serverActionStatus.errorData) ||
      this.props.statusFetchError
    ) {
      return (
    ...
    return (
      <div className="startButtonContainer">
        <button
          type="button"
          className="button btn-dark btn-block btn-lg shadow my-5 h-50 p-4 bg-dark rounded align-middle min-height-button"
          disabled={isDisabled}
          onClick={this.clickHandler.bind(this)}
        >
          {text}
        </button>
        {/* <div className="serverTypeContainer">
          <p className="serverTypeButton">
            <a>Server Type</a>
          </p>
          <div className="checkboxDiv">
            <input type="checkbox" name="m5.large" title="m5.large"></input>
            <label>m5.large</label>
            <input type="checkbox" name="m5.xlarge" title="m5.xlarge"></input>
            <label>m5.xlarge</label>
          </div>
        </div> */}
      </div>
    );
