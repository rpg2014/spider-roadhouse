...
import { memory } from 'roadhouse-wasm/roadhouse_wasm_bg';
      ...
      <main role="main" className="inner cover-container col cover mt-3 mb-auto container ">
        <div className="row p-1 rounded transparency">
          <p className="lead text-center text-dark m-0 non-transparent">
            ...
            <a
              ...
              className="text-muted non-transparent"
            >
              wasm-game-of-life
            </a>{' '}
            rust tutorial.
        ...
        <div className="row">
          <canvas
            ...
            className="col-sm m-4 justify-content-center"
            width={this.CANVAS_WIDTH}
            height={this.CANVAS_HIGHT}
            ref={this.canvasRef}
          />
        </div>
        <div className="row">
          <button onClick={this.handleClick} className="m-3 col justify-content-center button btn-dark btn-lg shadow">
            Reset
          </button>
        </div>
      </main>
  ...
  drawCells = () => {
    ...
    for (let row = 0; row < this.height; row++) {
      for (let col = 0; col < this.width; col++) {
        ...
        this.state.ctx.fillStyle = cells[idx] === Cell.Dead ? DEAD_COLOR : ALIVE_COLOR;

        this.state.ctx.fillRect(col * (CELL_SIZE + 1) + 1, row * (CELL_SIZE + 1) + 1, CELL_SIZE, CELL_SIZE);
      }
    }
