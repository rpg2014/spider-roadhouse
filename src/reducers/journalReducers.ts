import {
  ...
  FETCH_JOURNAL_ENTRIES_DONE,
  CREATE_JOURNAL_ENTRY,
  CREATE_JOURNAL_ENTRY_DONE,
  TOGGLE_NEW_DIALOG,
  DELETE_ENTRY,
...
export function fetchJournalEntriesReducer(
  fetchingState: IFetchingState<JournalEntryProps[]> = initalJournalEntrys,
  action: IAction<JournalEntryProps[], any>
): IFetchingState<JournalEntryProps[]> {
  switch (action.type) {
...
export function createEntryReducer(
  fetchingState: IFetchingState<boolean> = initalDeleteEntry,
  action: IAction<boolean, Partial<JournalEntryProps>>
): IFetchingState<boolean> {
  switch (action.type) {
